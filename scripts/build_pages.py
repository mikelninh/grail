...
def main() -> int:
    ...
    errors=list(payload.get("errors",[]))+sale_errors; print(json.dumps({"candidates":len(payload.get("candidates",[])),"counts":payload.get("counts",{}),"sales":len(sales),"premium_signal_types":len(premiums),"inventory":inventory.get("counts",{}),"graph":graph["stats"],"hunter_assets":True,"research_page":True,"errors":errors},indent=2)); return 0
